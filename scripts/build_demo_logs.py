...
# - nd_hybrid + lidar+depth (headline, pre-): _08,09,10 = /e/f
# (post- headline). Awaiting /h (post-) after charge.
# - nd_hybrid + lidar-only TRUE: _03,04 = / (post-)
# - nd_hybrid + depth-only NEW: _01,02 = /b (post-)
# - nd_hybrid + tof_off + v_max=0.05: _03,04 = / (post-, 150 s)
# - nd_only + lidar+depth: _01,02 = / (post-)
# - vfh_plus + lidar+depth (pre-): _01,02 = /
KEEPERS = [
    # cell                          trial_id                                                 label
    ("nd_hybrid_lidar+depth_pre_B11",  "C_integrated_nd_hybrid_lidar_depth_08",     "replicate"),
    ("nd_hybrid_lidar+depth_pre_B11",  "C_integrated_nd_hybrid_lidar_depth_09",     "replicate"),
    ("nd_hybrid_lidar+depth_pre_B11",  "C_integrated_nd_hybrid_lidar_depth_10",     "replicate"),
    ("nd_only_lidar+depth",            "C_integrated_nd_only_lidar_depth_01",       "replicate"),
    ("nd_only_lidar+depth",            "C_integrated_nd_only_lidar_depth_02",       "replicate"),
    ("vfh_plus_lidar+depth_pre_B11",   "C_integrated_vfh_plus_lidar_depth_01",      "replicate"),
    ("vfh_plus_lidar+depth_pre_B11",   "C_integrated_vfh_plus_lidar_depth_02",      "replicate"),
    ("nd_hybrid_lidar_only_TRUE",      "C_integrated_nd_hybrid_lidar_only_03",      "replicate"),
    ("nd_hybrid_lidar_only_TRUE",      "C_integrated_nd_hybrid_lidar_only_04",      "replicate"),
    ("nd_hybrid_depth_only_NEW",       "C_integrated_nd_hybrid_depth_only_01",      "replicate"),
    ("nd_hybrid_depth_only_NEW",       "C_integrated_nd_hybrid_depth_only_02",      "replicate"),
    ("nd_hybrid_tof_off_safety",       "C_tof_off_safety_nd_hybrid_lidar_depth_03", "replicate"),
    ("nd_hybrid_tof_off_safety",       "C_tof_off_safety_nd_hybrid_lidar_depth_04", "replicate"),
]
...
def main() -> int:
    ...
    lines.append("")
    lines.append(f"Generated {pathlib.Path(__file__).name}. Universal protocol: **150 s drive window, N ≥ 2 replicates per cell**. Paths aligned in `aligned_paths_grid.png` and `aligned_paths_overlay.png` with start = (1, 1) in arena frame, dead-end at (0, 0).")
    lines.append("")
