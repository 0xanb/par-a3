...
wait_motors_ready() {
 ...
 # diff_drive_controller never spawned. Per line 14 +
 # lines 15-18, the authoritative signal is /joint_states
...
set_cpu_policy() {
  ...
  if [[ "$1" == "d" ]]; then
    ...
    echo "[scene] mode D: killing qr_detector + qr_command_interpreter for the session (-60% CPU)"
    ssh "$ROBOT" 'pkill -f par_qr_nav/lib/par_qr_nav/qr_detector 2>/dev/null; true' 2>/dev/null || true
    # The interpreter latches a standby STOP@96 in mode A and republishes it at
    # 10 Hz; left alive it outranks every gesture intent (gesture STOP is p=85)
    # and pins the chassis ("detection works but robot does not move"). The mode
    # gate in command_interpreter.py also suppresses this, so this kill is
    # belt-and-braces + frees the node's CPU. Both nodes are plain (no respawn /
    # on_exit), so killing them does not cascade a runtime restart.
    ssh "$ROBOT" 'pkill -f par_qr_nav/lib/par_qr_nav/command_interpreter 2>/dev/null; true' 2>/dev/null || true
    ...
    # Restore qr_detector if mode-D killed it. Probe first so a repeated
    # `scene.sh a` (or coming from IDLE) does not waste ~30-40 s on a needless
    # runtime restart. We deliberately do NOT restore the snap husarion-webui
    # bridge (8765): it is redundant, not par_msgs-aware, and burns CPU + spams
    # the log. The operator's foxglove is our own overlay-aware bridge on 8766,
    # restored by the par-a3-runtime restart below; par_a3_runtime.sh also stops
    # the snap bridge on every boot.
    local probe
    probe="$(ssh -o ConnectTimeout=5 "$ROBOT" '
      pgrep -f par_qr_nav/lib/par_qr_nav/qr_detector >/dev/null && echo "qr=yes" || echo "qr=no"
    ' 2>/dev/null)" || probe="qr=unknown"

    if [[ "$probe" != *"qr=yes"* ]]; then
      echo "[scene] mode A: restarting par-a3-runtime (brings qr_detector + 8766 foxglove back) [${probe}]"
...
publish_mode() {
 ...
 # silent, planner gates stay closed. Verified live during attempt.
...
stop_scenes() {
 ...
 # Post 2-mode pivot : par_reactive_nav and par_gesture are part
  ...
  # legacy per-scene launches (b/c/d, par_traffic_light) and stray ros2
  ...
  ssh "$ROBOT" '
    ...
    sweep "/par_traffic_light/lib/par_traffic_light/"
...
case "$SCENE" in
 ...
 # RGB-only for CPU budget; post- depth fusion is the dominant signal
 ...
 # /buttons row removed post- — that topic does not exist on the
