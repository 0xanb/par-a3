 ...
 *,
 ...
 Returns True when:
 1. candidate is not None (classifier produced a verb),. 2. candidate equals pending_label (same pose held since pending_since_s),. 3. (now_s - pending_since_s) >= hold_seconds (held long enough),. 4. (now_s - last_fired_s) >= cooldown_s (cooldown elapsed since last fire).
...
class GestureDetector(Node):
 ...
 # matches the legacy hold_ticks=2 at rate_hz=5 (production).
 ...
 # 2-mode pivot : default_mode="IDLE" means inference does
 ...
 # Emit a TrialEvent so the par_eval snapshotter captures the frame. # session_logger records the fire. The snapshotter triggers on
