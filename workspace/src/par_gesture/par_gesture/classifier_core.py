...
Replaces the body-pose classifier (archived ) with a
...
# samples under The rule below requires the
# wrist→middle-MCP axis to be predominantly horizontal (|dx| > |dy|) and
# the middle-MCP at least _PALM_POINT_MIN of frame width offset from the
...
def gun_pointing_left(lm) -> bool:
 ...
 demo-eve session — see 07-LOG.md."""
    if not all_fingers_extended(lm):
        return False
