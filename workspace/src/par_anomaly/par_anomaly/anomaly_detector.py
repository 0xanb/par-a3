...
* **Collision impact** — subscribes ``/imu_broadcaster/imu``
  ...
  wheels stop within 0.5 s of the spike (suppresses rumble strips and
  thresholds). 1.0 s emit cooldown to prevent spam.
* **Wheel stall** — subscribes ``/cmd_vel`` (post-clamp) and
  ``/odometry/filtered`` (EKF chassis velocity), publishes
...
def _build_node():
    ...
    class AnomalyDetector(Node):
        def __init__(self) -> None:
            ...
            # Calibrated evening: was 8.0; at slow cruise
