...
def verb_to_intent(
    ...
    When ``restrict_to`` is None (default) all seven verbs resolve normally and
    unknown verbs return None — preserves the original contract.
...
# — RECOVERING is shaped as a one-shot
...
def any_source_moving(latest: dict[str, tuple[float, float]],
    ...
    Matches the arbiter's ``stale_after_s=0.5`` drop window so interpreters and
    arbiter agree on what 'currently moving' means."""
