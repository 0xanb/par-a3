...
# via the `mode_a_allowed_verbs` ROS param. STOP and GO only — turns and
# speed adjusters are rejected to keep the demo flow predictable.
...
#
# Mapping onto the PDF §A.1.4 four-state vocabulary, per
# :
...
class CommandInterpreter(Node):
    ...
    def _on_active_mode(self, msg: ActiveMode) -> None:
        ...
        if new_mode and new_mode != self._active_mode:
            ...
            # Scene A standby (; re-arm every entry ).
            # On EVERY transition into mode A, latch a STOP intent at the QR
            # stop priority (STOP_PRIORITY=96) so reactive nav (p=70) does NOT
            # start driving the chassis the moment the gate flips. The operator
            # must explicitly show a GO card to release this latch — the
            # demo-day "robot stays parked until I say go" model, now applied on
            # idle->A and D->A alike, not only the first cold-boot entry. Before
            # this, the `self._current is None` guard meant any prior GO left
            # _current non-None, so re-entering A let reactive cruise on its own.
            # The latch is replaced cleanly by any subsequent QR verb via
            # _on_detect. _phase guard never clobbers an in-flight maneuver
            # (turns are rejected in mode A; belt-and-braces for future verbs).
            # The continuous cold_boot_auto publisher (par_a3_runtime.sh, 1 Hz)
            # cannot re-park a cruising robot: the new_mode != self._active_mode
            # guard above no-ops repeated A messages.
            if new_mode == "A" and self._phase == _PHASE_IDLE:
    ...
    def _republish(self) -> None:
        # Mode gate: the QR channel drives the chassis only in mode A. In mode D
        # the gesture channel owns control and in IDLE/boot everything stays
        # quiet, so a standby STOP@96 (or any verb) latched while in mode A must
        # NOT keep republishing after a mode switch — it outranks every gesture
        # intent (gesture STOP is p=85) and pins the robot. Mirrors
        # gesture_interpreter's ModeState("D") gate. The recovering one-shot in
        # _on_detect bypasses this gate by design, but it is priority 0 and so
        # cannot block any real intent.
        if self._active_mode != "A":
            return
        now = time.monotonic()
