...
def test_find_valleys_ignores_short_run_between_two_blocked_segments -> None:
 ...
 valleys = find_valleys(mask, cfg)
 # Only the 48 run (length 5) should be kept.
...
def test_select_heading_prefers_forward_valley -> None:
 ...
 # With everything free we get one big valley spanning 0n-1. Its centre is
...
def test_select_heading_takes_forward_when_open_even_if_shallow -> None:
 ...
 # Shallow valley containing the forward bin (02) — clearance just above
 # the 0.50 m threshold.
 for i in range(0, 3):
 hist[i] = 0.55
 # A deeper alternative valley (bins 57) at ~5 m. Old logic would have
...
def test_select_heading_picks_deepest_when_forward_blocked -> None:
 ...
 # Two valleys, neither containing forward bin 0. Bins 57 = 0.55 m
 # (shallow), bins 3032 = 5.0 m (deep). Forward (bin 0) sits in the
...
def test_select_heading_hysteresis_switches_when_clearly_better -> None:
 ...
 depth_bonus_weight=8.0)
 hist = np.full(72, 0.30, dtype=np.float32)
 # Incumbent valley at bins 1820 (centre 19), shallow (0.55 m).
 for i in range(18, 21):
 hist[i] = 0.55
 # New deep valley at bins 57 (centre 6), 5 m. Forward bin 0 is in
...
def test_blocked_mask_inflates_thin_obstacle_at_close_range -> None:
 ...
 # bins each side, plus the centre bin → 73 bins blocked. With wrap. # the cap, expect the entire histogram blocked.
 assert mask.sum >= 70, (
