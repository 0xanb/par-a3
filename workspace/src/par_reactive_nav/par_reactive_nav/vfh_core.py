...
def should_emit_stale_event(
    ...
    *,
    ...
    Returns True iff:
      - we have seen at least one timestamp (or were primed at boot), AND
      - the most recent histogram is older than ``stale_threshold_s``, AND
      - either we never emitted a stale event before, or the previous one is
