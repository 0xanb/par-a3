...
`/par/intents` publication contract (source="reactive", priority 70). Post-hybrid: emits `DEAD_END_LS2` (geometric dead-end) and `DEAD_END_WEDGE`
...
class NDPlanner(Node):
 ...
 #: lowered from 0.18 → 0.12 for the demo arena.
 ...
 # with the slipper-in-front session.)
