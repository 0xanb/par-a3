...
in the post- hybrid so the recovery_controller can pattern-match on
labels starting with "DEAD_END" — .)
...
class NDConfig:
    ...
    cruise_v: float = 0.12             # nominal forward speed (m/s); lowered evening for gentler-impact demo arena
...
def _bin_to_yaw(chosen_bin: int, cfg: NDConfig) -> float:
    ...
    n = cfg.n_bins
    # Bins 0..n/2 are positive-yaw (left-handed convention used by /scan).
